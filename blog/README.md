...
```bash
pnpm dev          # Notebook 변환 후 개발 서버
pnpm check        # Astro 타입·콘텐츠 스키마 검사
pnpm test         # URL·콘텐츠 계약 단위 테스트
pnpm build        # 전체 검사, 정적 빌드, Pagefind, 링크 검사
pnpm preview      # /playground/ production preview
```
...
publish는 깨끗한 `blog/` 작업 트리를 확인하고 전체 build를 다시 수행한 다음 `dist/`의 내용만 `gh-pages` 브랜치 루트에 게시한다. force/orphan history 재작성은 사용하지 않으며 source commit SHA를 배포 commit 메시지에 기록한다. 배포 전 로컬 게이트만 확인하려면 `pnpm run publish:dry-run`을 사용한다.

## 롤백

배포 후 치명적인 문제가 있으면 Git 이력에서 마지막 정상 source commit과 `gh-pages` commit을 복구하고, 원인과 누락 검사를 기록한 뒤 다시 배포한다.
