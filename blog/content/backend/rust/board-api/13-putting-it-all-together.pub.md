...
| 배경 | 추천 |
|------|------|
| **Rust를 전혀 모름** | 00 → 01~03을 천천히 → `cargo run`으로 00 실습 → 04~07 → 나머지 |
| **Rust는 알지만 웹 프레임워크가 처음** | 00, 04~12를 중심으로, 01~03은 참고 |
| **백엔드 시니어** | 00·07·08·13 + 소스 직접 대조. 각 편 **「심화」** 절 위주 |

막히면 **컴파일러 에러 메시지를 그대로 검색**하는 것이 Rust 학습에서 가장 빠른 길입니다. `cargo check` 출력은 “어느 파일·어느 타입이 안 맞는지”를 정확히 알려 줍니다.
...
## 심화: 프로덕션으로 올릴 때 체크리스트

| 항목 | 학습용 `board-api` | 개선 방향 |
|------|-------------------|-----------|
| DB | SQLite 파일 | Postgres, connection URL 시크릿 |
| 스키마 | `init_schema` | `sea-orm-cli migrate` |
| 에러 노출 | JSON에 영문 `error` | 로그는 상세, 클라이언트는 코드만 |
| CORS | permissive | 허용 origin 화이트리스트 |
| 인증 | 없음 | 세션/JWT middleware |
| 관측 | tracing 로그 | metrics, trace id, health beyond `/health` |
| 생성 응답 | 200 + body | 201 + `Location` |

## 자주 묻는 질문 (FAQ)

**Q. Rust 없이 Spring으로만 하면 안 되나요?**  
A. 물론입니다. 이 시리즈는 **Rust + Axum 생태계 읽기**가 목표입니다.

**Q. 컴파일이 너무 느려요.**  
A. 첫 빌드는 의존성 전체 컴파일입니다. `cargo check`로 개발하고, `cargo build --release`는 배포 전에만 쓰면 됩니다.

**Q. `?`가 뭔지 아직 헷갈려요.**  
A. “실패하면 여기서 함수 끝내고 Err를 위로 넘긴다”로 기억하세요. 03·08편 예제를 `return Err(...)`로 풀어 써 보면 같습니다.
