...
인라인 수식은 `$...$`, 블록 수식은 `$$...$$`로 쓴다. 빌드할 때 KaTeX가 정적
HTML과 MathML로 렌더링하므로 브라우저에서 실행되는 스크립트는 없다. KaTeX
스타일시트는 수식이 있는 글에서만 불러온다.

```markdown
독립 변수 $x$로 종속 변수 $y$를 예측한다.
...
코드 블록과 인라인 코드 안의 `$`는 수식으로 해석되지 않으므로 `$1` 같은 SQL
바인딩이나 셸 변수는 그대로 쓰면 된다. 본문에서 달러 기호를 글자 그대로
써야 할 때는 `\$`로 이스케이프한다.
