...
- 공개 주소는 `https://kim-hyunjin.github.io/playground/`이며 Astro가 콘텐츠 경로에서 정적 글 URL을 생성한다.
...
마이그레이션 대상은 Markdown 130개와 Notebook 21개다. Astro 5.2 이상에서는 Tailwind CSS 4의 Vite 플러그인이 공식 권장 방식이며, 과거 `@astrojs/tailwind` 통합은 사용하지 않는다.
...
- 기존 URL 호환성은 요구하지 않으며, build는 현재 출력의 필수 파일·canonical·내부 링크·에셋 무결성만 검증한다.
