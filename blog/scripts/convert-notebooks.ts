...
import { joinNotebookSource, titleFromNotebook } from './markdown-headings.ts';
import { parseNotebook, type Notebook, type NotebookOutput } from './notebook-types.ts';
...
interface ConvertNotebookOptions {
  sourcePath: string;
  date?: string;
  sourceModified?: string;
  outputAssetRoot?: string;
}
 ...
 */
function plainSummary(notebook: Notebook, title: string): string {
 ...
 */
function gitValue(path: string, format: string): string {
  try {
 ...
 */
function normalizedCategory(sourcePath: string): string {
 ...
 */
function frontmatter(data: NotebookFrontmatter): string {
  /**
   * 문자열 배열을 YAML 블록 배열 형식으로 직렬화한다.
   */
  const yamlArray = (values: string[]): string =>
...
export async function convertNotebook(
  ...
  }: ConvertNotebookOptions,
): Promise<string> {
  const notebook = parseNotebook(notebookInput);
  const fileName = sourcePath.split('/').at(-1) ?? sourcePath;
  const fallbackTitle = fileName.replace('.pub.ipynb', '').replaceAll('-', ' ');
 ...
 */
export async function convertNotebooks(): Promise<void> {
  ...
  for (const sourceFile of files) {
    ...
    const notebook: unknown = JSON.parse(await readFile(sourceFile, 'utf8'));
