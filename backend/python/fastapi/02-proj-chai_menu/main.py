...
def get_menu(category: str = Query(None, description="Filter menu items by category (e.g., 'chai', 'herbal')")):
    from data import menu_items
    filtered_items = [item for item in menu_items if category is None or item["category"] == category]
    if not filtered_items:
        raise HTTPException(status_code=404, detail=f"No menu items found in category: {category}")
