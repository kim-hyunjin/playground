...
describe('TradingChart Component', () => {
  it('renders the chart container and basic UI elements', () => {
    ...
    // Header
    expect(screen.getByText(/거래소 차트 \(드래그 & 줌\)/)).toBeInTheDocument();
    expect(screen.getByText(/드래그로 이동, 휠로 확대\/축소가 가능합니다\./)).toBeInTheDocument();
    
    // Initial instruction
    expect(screen.getByText(/차트에 마우스를 올려 상세 시세를 확인하세요/)).toBeInTheDocument();
